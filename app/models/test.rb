...
# binding.pry
  (0...@links.count).collect{|i| Apartment.find_or_create_by(links: @links[i], locations: @location[i], pictures:@picture[i], price_range:@price_range[i], bed_rooms: @bed_rooms[i] ) }
  Apartment.where(:links => 'favoriteIcon neutral').destroy_all

  # Apartment.apartments_show_scraper(@links)
  end

  def apartments_show_scraper(show_page_links)
  #### show page
    @transportation = []
    # @sqft = []
...
######
def self.naked_apartments_scraper(borough)
  manhattan = 'http://www.nakedapartments.com/renter/listings/search?nids=23%2C211%2C6%2C21%2C203%2C191%2C194%2C18%2C24%2C76%2C204%2C205%2C10%2C14%2C195%2C1%2C5%2C25%2C93%2C206%2C22%2C17%2C207%2C13%2C155%2C16%2C72%2C2%2C9%2C20%2C19%2C73%2C7%2C208%2C209%2C192%2C8%2C74%2C210%2C11%2C4%2C3%2C26%2C212%2C12&order=asc&page=1&sort=rent'
     brookyn = 'http://www.nakedapartments.com/renter/listings/search?nids=117%2C77%2C46%2C29%2C78%2C36%2C79%2C118%2C30%2C119%2C41%2C120%2C37%2C31%2C38%2C213%2C121%2C45%2C122%2C32%2C33%2C123%2C124%2C82%2C83%2C84%2C34%2C214%2C39%2C126%2C43%2C127%2C85%2C215%2C216%2C129%2C95%2C130%2C27%2C28%2C128%2C88%2C40%2C217%2C132%2C89%2C35%2C44%2C47&order=asc&page=1&sort=rent'
  ...
  page = Nokogiri::HTML(open(borough))
...
(0...@show_page_links.length).each{|i| Apartment.find_or_create_by(links: @show_page_links[i], locations: @street_address[i], pictures:@pictures[i], bed_rooms: @bedrooms[i] ) }
